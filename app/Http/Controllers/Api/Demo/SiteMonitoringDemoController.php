...
use App\Services\Demo\SiteMonitoringDemoService;
use App\Support\DemoGuestSession;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Symfony\Component\HttpFoundation\Cookie;

class SiteMonitoringDemoController extends Controller
{
    public function run(Request $request): JsonResponse
    {
        $cfg = SiteMonitoringDemoService::config();
        $module = SiteMonitoringDemoService::MODULE;
        ...
        if ($remainingBefore <= 0) {
            return $this->jsonError(
                ...
                [
                    'error' => 'rate_limit',
                    'message' => 'Лимит проверок в демо на сегодня исчерпан. Зарегистрируйтесь — мониторинг по расписанию и оповещения.',
                    'remaining' => 0,
                ],
                $guest['guestId'],
                $guest['state'],
        ...
        $validated = SiteMonitoringDemoService::validate([
            'url' => $body['url'] ?? '',
            'phrase' => $body['phrase'] ?? '',
            'waiting_time' => $body['waiting_time'] ?? null,
        ]);
        ...
        $result = SiteMonitoringDemoService::check($validated);
        ...
        $payload = SiteMonitoringDemoService::buildResponse($result, $bump['remaining'], $guest['guestId']);
