...
@section('title', __('Site monitoring report'))
...
@section('content')
    <div class="alert alert-info cabinet-sm-public-banner mb-3">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-header py-2 d-flex flex-wrap align-items-center gap-2">
            <h1 class="card-title h5 mb-0">
                <i class="bi bi-globe2 me-1 text-primary"></i>{{ __('Monitored domains') }}
            </h1>
            <span class="badge text-bg-secondary">v{{ $shareMeta['version'] ?? config('cabinet-site-monitoring.version', '1.0') }}</span>
            @if(!empty($shareMeta['generated_at']))
                <span class="small text-secondary ms-auto">{{ __('Generated') }}: {{ $shareMeta['generated_at'] }}</span>
            @endif
        </div>
        <div class="card-body cabinet-site-mon-page p-3">
            <p class="mb-3">
                <strong>{{ $report['project']['name'] ?? '—' }}</strong><br>
                <a href="{{ $report['project']['link'] ?? '#' }}" target="_blank" rel="noopener noreferrer">{{ $report['project']['link'] ?? '' }}</a>
            </p>
            @include('site-monitoring.partials.stats-report-body', [
                'report' => $report,
                'isPublicView' => true,
            ])
        </div>
    ...
    <div class="text-center mb-4">
        <a href="{{ \App\SiteMonitoringPublicShare::registerUrl() }}" class="btn btn-primary">
